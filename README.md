...
SightMate 是一个 Web 端 AI 视觉对话助手。用户打开摄像头后，可以通过连续语音对话或文字提问；系统在用户说完一句话或主动发送问题时无感截取当前画面，将问题和截图发送到后端，由阿里云百炼千问视觉模型生成回答，并在前端展示和语音播报。
...
- 浏览器语音识别输入。
- 文字输入兜底。
- 连续对话模式：说完一句话后自动截取当前画面并提问。
- 文字提问模式：点击发送时自动截取当前画面。
