...
SightMate 是一个 Web 端 AI 视觉对话助手。用户打开摄像头后，可以通过语音或文字提问；系统在用户主动发送问题时截取当前画面，将问题和截图发送到后端，由阿里云百炼千问视觉模型生成回答，并在前端展示和语音播报。
...
- 后端：FastAPI、Python
- 模型服务：阿里云百炼千问视觉模型，例如 `qwen-vl-plus`
- 语音能力：浏览器 Web Speech API、`speechSynthesis`
...
| `ALIYUN_VISION_MODEL` | 视觉模型名称，默认 `qwen-vl-plus` |
| `BACKEND_HOST` | 后端监听地址 |
| `BACKEND_PORT` | 后端监听端口 |
...
多模态请求中，图片通过 `image_url.url` 传入 Base64 Data URL。前端不会直接请求阿里云 API，也不会暴露 `ALIYUN_API_KEY`。

## 隐私说明

SightMate 不上传连续视频流。摄像头画面只在浏览器中实时预览，只有用户点击发送问题时，系统才截取当前帧并发送给后端模型服务。

历史记录保存在浏览器 localStorage 中，不上传到云数据库。清空浏览器站点数据或点击清空历史会删除本地历史记录。
