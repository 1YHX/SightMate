...
- 摄像头：前端通过 `getUserMedia` 打开摄像头，本地实时预览，并向实时模型发送低帧率压缩画面。
- 麦克风：前端通过 `getUserMedia` 打开麦克风，将音频转为 16k PCM 发送给实时模型。
- AI 回答：实时模式使用 Qwen-Omni-Realtime 返回流式文字和语音；实时不可用时回退到锁帧视觉问答和 Qwen-TTS 播报。
- 历史记录：SQLite 完整保存本地多轮对话会话，支持查看、删除单个会话和清空全部历史。
- 成本控制：不上传原始高帧率视频流，只发送约 1fps 压缩帧；模型上下文只带最近 3 轮。
- 设计文档：见 [docs/DESIGN.md](docs/DESIGN.md)，包含计划/最终实现的用户故事，以及计划/实际采用的成本控制策略。
